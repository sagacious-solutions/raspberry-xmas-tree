...
sio = socketio.Server(
    logger=True, async_mode=async_mode, cors_allowed_origins="*"
)
app = Flask(__name__)
...
@sio.event
def set_color(sid, message):
    print("Got set color", file=sys.stdout)
    sio.emit("my_response", {"data": "Left room: " + message["room"]}, room=sid)
...
if __name__ == "__main__":
    if sio.async_mode == "threading":
        ...
        if websocket:
            pywsgi.WSGIServer(
                ("", PORT), app, handler_class=WebSocketHandler
            ).serve_forever()
        else:
